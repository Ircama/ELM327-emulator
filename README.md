...
- `scenario <scenario>` = switch to `<scenario>` scenario; if the scenario is missing or invalid, defaults to `'test'`. The autocompletion allows prompting all compatible scenarios defined in `emulator.ObdMessage`.
...
The file produced by *obd_dictionary.py* can be dynamically imported in *python-ELM* through the *import* command and can then be merged in *ObdMessage* via `emulator.ObdMessage.update(self.module.ObdMessage)`. Example:
...
Notice that autocompletion is allowed for `import`, for the `import` argument, for `scenario`, `scenario` argument (which will include the new *Auris* name) as well as variables.
